...
_Objtype = _grub select (random (count _grub - 1));
...
_food = _grub select floor (random (count _grub)) createvehicle getpos c130;
...
sleep 0.1;
		deTach _food;
		_food setPos [(getPos _food select 0),(getPos _food select 1),(getPos _food select 2)-0];
_food setVariable["food",20,true];
		...
		_food attachTo [_Parachute,[0,0,-0.0]];
